...
import { getAvatar, generateAvatar, updateAvatar, updatePerson, addTeam, getMyTeam, getProfile } from "@/router/api";
...
const InfoManage: React.FC = () => {
  ...
  const isFormValid = () => {
    return (
      TeamTitle !== null && TeamTitle.trim() !== '' &&
      TeamDiscribe !== null && TeamDiscribe.trim() !== '' &&
      TeamRequest !== null && TeamRequest.trim() !== '' &&
      TeamTel !== null && TeamTel.trim() !== '' &&
      TeamNowNumber !== null &&
  ...
  const handleUpdateTeam = async (updateTeam: Team) => {
    try {
      await updatedTeamAPI(token as string,updateTeam.id as number,updateTeam.teamname,updateTeam.Now,updateTeam.MAX,updateTeam.introduction,updateTeam.require as string,updateTeam.contact as string);
  ...
  const handleSubmitTeamAdd = async () => {
    try {
      await addTeam(token as string, TeamTitle as string, TeamNowNumber as number, TeamTotalNumber as number, TeamDiscribe as string, TeamRequest as string, TeamTel as string);
      fetchTeams();
    }
    catch (error) {
                      ...
                      <div className="flex flex-col gap-4">
                        ...
                        <div className="mt-2 flex justify-center gap-10 w-full">
                          <Dialog.Close asChild>
                            <button className={styles.searchButton} onClick={handleSubmitUpdate}>
                              提交表单
                            </button>
                          </Dialog.Close>
                        </div>
              ...
              <Dialog.Root>
                ...
                <Dialog.Portal>
                  ...
                  <Dialog.Content className="fixed top-1/2 left-1/2 w-96 -translate-x-1/2 -translate-y-1/2 rounded-lg bg-white p-6 shadow-lg">
                    ...
                    <Dialog.Close asChild>
                      <button title="关闭" className="absolute top-2 right-2 p-1 text-gray-500 hover:text-gray-700">
                        <Cross2Icon />
                      </button>
                    </Dialog.Close>
                  </Dialog.Content>
