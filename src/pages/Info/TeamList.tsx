...
const TeamList: React.FC<TeamProps> = ({ Teams,onUpdateTeam}) => {
  ...
  const handleNumberChange = (e: React.ChangeEvent<HTMLInputElement>, field: 'Now' | 'MAX') => {
    if (!currentTeam) return;
                  ...
                  <Dialog.Root>
                    <Dialog.Trigger asChild>
                      ...
                      <div className="px-3 py-1 rounded absolute bottom-4 right-4">
                        <button className={styles.searchButton} onClick={()=>handleEditClick(team)}>
                          编辑
                        </button>
                      </div>
                    </Dialog.Trigger>
                    {currentTeam && (<Dialog.Portal>
