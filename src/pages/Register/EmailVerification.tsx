  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_authURL}/verify-complete`,
        {
          method: "POST",
          headers: {
      ...
      if (data.err_code === 200000) {
        showResult(
          "邮箱验证成功！您的账号已创建，3秒后将跳转到登录页面...",
          true,
        );
        setTimeout(() => {
    ...
    <>
      <div className={style.authContainer}>
        <div className={style.authCard}>
          <h1>邮箱验证</h1>
          <p>
            您正在验证 Mundo 账号的邮箱地址。请设置您的账号密码并完成人机验证。
          </p>
          <div className={style.authCard}>
            <form onSubmit={handleSubmit}>
              ...
              {state.message && (
                <div
                  className={`result ${state.isSuccess ? "success" : "error"}`}
                >
                  {state.message}
                </div>
              )}
